...
internal class BlurTextureSource(
    viewportSize: Size,
    ...
    private val blurShader by lazy {
        val handle = ShaderCreator.createShader(
            ...
            shadersCodeRepositoryProvider().getShaderCode(GLShaderId.Blur).value
        )
    ...
    override fun draw(forceDraw: Boolean, inputTexture: Int): Boolean {
        ...
        if (forceDraw) {
            blurShader.inputTexture = inputTexture
            blurShader.draw(viewportSize)
        }
    ...
    class Factory(
        ...
        override fun create(viewportSize: Size) = BlurTextureSource(
            viewportSize,
            shadersCodeRepositoryProvider
        )
    }
