...
abstract class AbstractShaderGLRenderer : GLSurfaceView.Renderer {
    ...
    private var totalFrameTimes: Long = 0
    ...
    override fun onSurfaceCreated(p0: GL10?, p1: EGLConfig?) {
        ...
        log("onSurfaceCreated ${Thread.currentThread().name}")

        val shaderNanos = measureTime {
            programId = createShaderProgram(
                vertexShader,
                fragmentShader,
            )
        }.inWholeNanoseconds
        log("createNanos $shaderNanos")
    ...
    override fun onSurfaceChanged(gl: GL10?, width: Int, height: Int) {
        log("onSurfaceChanged ${Thread.currentThread().name}")
    ...
    override fun onDrawFrame(p0: GL10?) {
        ...
        val frameTime = finish - start

        frames++
        totalFrameTimes += frameTime
        if (finish - lastTime > 1_000_000_000) {
            lastTime = finish
            val fps = frames
            val avgFrameTimeMs = totalFrameTimes / 1_000_000f / frames
            frames = 0
            totalFrameTimes = 0
            log("FPS: $fps, frame time: $avgFrameTimeMs")
        }
    }
